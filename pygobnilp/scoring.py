...
class fNML(AbsDiscreteLLScore):
    ...
    def createCTable(self,N,K):
        ...
        Table is generated by first computing the index 2 column and then using a recusive formula to fill the rows up to K.
        ...
        logs = np.zeros(N+1, dtype=np.float64)
        logfacs = np.zeros(N+1, dtype=np.float64)
    ...
    def szpankowskiApproximationLargeK(self, row, column):
        '''
        A third implementation of szpankowskiApproximation, this one also from bnlearns implementation. This one is specially for large values of K
