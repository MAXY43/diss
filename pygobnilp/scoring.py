...
class fNML(AbsDiscreteLLScore):
    '''
    Extension of the Discrete likelihood score with an extra penalty from fNML.
    '''

    def __init__(self, data):
        '''
        Initialises a fNML object.
        Args: data (DiscreteData) : the data
        '''
        _AbsLLPenalised.__init__(self, data)
        self._child_penalties = {}
        self._ctable = None


    def score(self, child, parents):
        '''
        Return the fNML score for a 'child' having 'parents'. No upper bound currently.
        Args:
            child (str): The child variable
            parents (iter) : The parents
        Returns:
            Tuple, the first element containing the fNML score for a given family and currently None for the upper bound.
        '''
        this_ll_score, numinsts = self.ll_score(child, parents)
        counts = self.find_counts(parents)
        self.cal_child_penalties(child, parents, counts)
        penalty = self._child_penalties[child]
        '''
        if not parents:
            print(parents)
            counts = self.fix_contab(self.data(), counts, child)
            score = self.log_likelihood_single_variable(counts)
            return score, None
        '''
    ...
    def cal_child_penalties(self,child, parents, counts):
        '''
        Calculate the child penalties for a given family and updated the dictionary storing child penalties.
        Args:
            child (str): The child variable
            parents (iter) : The parents
            counts (iter) : The counts calculated from the contingency table.
            data(DiscreteData) : the data
        Returns:
            Updates dictionary with the penalties for a child with parents calculated from the regret table.
        '''

        c_table = self.createCTable(self.data_length(),100)
    ...
    def find_counts(self, parents):
        '''
        Creates a contab from the given parent variables, reshaping it if is 1 dimensional so it can be iterated over.
        The contab is returned part of a tuple hence we extract the first element which contains the counts we need in the above score function.
        Args:
            parents (iter) : The parents
        Returns:
            contab (tuple) : The first element contains the counts as a ndarray and the second element contains the 'strides' of each column. (Variables)
        '''
        parents_list = list(parents)
        variables = parents_list
    ...
    def fix_contab(self, data, contab, var_name):
        '''
        Function to return the marginal contingency table for a single variable with no parents, not actually used in the end.

        Args:
            data (DiscreteData) : the data
            contab (np.ndarray) : the old contingency table
            var_name (str) : the name of the variable that you are calculating the marginal contingency table for
        Returns:
            contab (np.ndarray) : the marginal contingency table
        '''
        if contab.shape == (1, 1):
            var_data = data[var_name]
            unique_vals, counts = np.unique(var_data, return_counts=True)
            return counts
    ...
    def find_qi(self, parents):
        '''
        Calculated the qi value in order to sum over when finding child penalties. It is just the product of the arities of the parents.
        Args:
            parents (iter) : The parents
        returns:
            qi (int) : The qi value
    ...
    def createCTable(self,N,K):
        '''
        Generates the C (regret) table required to calculate the child penalties based on the size of the data, storing the table when
        created and only generating a new one when required too. Approximations are used for large N and K values to speed up computation.
        Table is generated by first computing the index 2 column and then using a recursive formula to fill the rows up to K.

        Args:
             N (int) : The required column size
             K (int) : The required row size
        Returns:
             C_table (ndarray) : The C (regret) table

        '''
        import numpy as np
        from scipy.special import gammaln

        logs = np.zeros(N+2, dtype=np.float64)
        logfacs = np.zeros(N+2, dtype=np.float64)
        for n in range(1, N+2):
            logs[n] = np.log(n)
            logfacs[n] = gammaln(n+1)
        ...
        if self._ctable is not None and self._ctable.shape[0] >= N + 1 and self._ctable.shape[1] >= K + 1:
            ...
            c_array = np.zeros((N+1,K+1), dtype=np.float64)
            for row in range(1, N+1):

                if row > 1000:
                    c_array[row, 2] = self.szpankowskiApproximation1(row, 2)
                else:
                    res = 0.0
                    for n in range(0, row+1):
                        term = logfacs[row] - logfacs[n] - logfacs[row - n]

                        if n > 0:
                            term += n * (logs[n] - logs[row])
                        if (N - n) > 0:
                            term += (row - n) * (logs[row - n] - logs[row])

                        res += np.exp(term)

                    c_array[row,2] = np.log(res)

            #recursion for columns greater than 2

            for row in range(1,N+1):
                for column in range(3,K+1):
                    if column > 100 or row > 1000:
                        value_1 = self.szpankowskiApproximationLargeK(row, column)
                        value_2 = self.szpankowskiApproximation1(row, column)
                        c_array[row,column] = min(value_1, value_2)
                    else:
    ...
    '''
    Currently redundent functions 
    
    def fill_col_upto_k(self, K, n, c_table):
        rk_prev_prev = 1
        rk_prev = np.exp(c_table[2, n])
        rk = 0.0

        for k in range(3, K + 1):
            rk = rk_prev + (rk_prev_prev / (k-2)) * n
            c_table[n, k] = np.log(rk)
            rk_prev_prev = rk_prev
            rk_prev = rk

        return c_table

    def regret_col2_get(self, logs, logfacs, N):
        res = 0.0
        for n in range(0, N):
            term = logfacs[N] - logfacs[n] - logfacs[N - n]
    ...
    def szpankowskiApproximation1(self, row, column):
        '''
        Function to calculate szpankowski approximation for large N, logic taken from bnlearn implementation.
        Args:
            row (int) : The row index
            column (int) : The column index
        Returns:
            (float) : Approximation of the regret value given a row and column index.
        '''
        import scipy.special as sp
        hK = 0.5 * column
        hk_h = hK - 0.5
        lgammahK = sp.gammaln(hK)
        lgammahk_h = sp.gammaln(hk_h)
        gamma_ratio = np.exp(lgammahK - lgammahk_h)
        gamma_ratio_term = np.sqrt(0.5) - gamma_ratio * column / (3* np.sqrt(row))
        if row == 0:
            return 0
        else:
            return (hk_h) * (np.log(row) - np.log(2)) \
                + np.log(np.sqrt(np.pi)) - lgammahK \
                + 0.5 - gamma_ratio_term ** 2 \
                + (3 + column * (column - 2) * (2 * column + 1)) / (36.0 * row)


    def szpankowskiApproximation2(self, n):
        '''
        Another implementation of szpankowskiApproximation, directly from the paper by (Silander et al., 2008), currently unused.
        This one is only relevant for the index 2 column.

        Args:
            n (int) : The row index
        Retuns:
            (float) : Approximation of a given row index for column index 2
        '''
        sqrt_term = np.sqrt(8 / (9 * n * np.pi) + (3 * np.pi - 16) / (36 * n * np.pi))
        return np.log((n * np.pi) / 2) + sqrt_term

    def szpankowskiApproximationLargeK(self, row, column):
        '''
        A third implementation of szpankowskiApproximation, this one is also from bnlearns implementation, specifically for large values of K.
        ...
        '''
        a = column / row
        Ca = 0.5 * (1 + np.sqrt(1 + 4 / a))

        return row * (np.log(a) + (a + 2) * np.log(Ca) - 1/Ca) - 0.5 * np.log(Ca + 2 / a)
